...
def fundamentals_check(ticker: str, max_retries: int = 2):
    ...
    # Kept to 2 attempts (not more) to avoid the whole page taking too
    # long across a large watchlist -- combined with the session-level
    # caching in fetch_fundamentals(), a stock that fails here simply
    # gets retried fresh on your next Refresh instead of being retried
    # many times in a single slow page load.
    info = None
    last_error = None
    for attempt in range(max_retries):
        try:
            ...
            last_error = str(e)
        time.sleep(0.8 + attempt * 0.8)  # 0.8s, 1.6s backoff between attempts

    if info is None:
        return {
            "status": "UNKNOWN",
            "flags": [f"Could not fetch fundamentals after {max_retries} attempts "
                      f"(likely a temporary rate-limit) — {last_error}. Click "
                      f"Refresh to retry — this specific stock is not cached "
                      f"as a failure, so it'll try fresh next time."],
            "data": {},
        }
